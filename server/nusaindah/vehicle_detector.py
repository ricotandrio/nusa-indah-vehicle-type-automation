...
def analyze_and_classify(signal, title="Signal", threshold=5):
  ...
  if len(tire_data) >= 2:
    ...
    for i, tire in enumerate(tire_data):
      ...
      plt.plot(t[tire['indices']], signal[tire['indices']], color=color, linewidth=3, alpha=0.8, label=f'Tire {i+1} ({tire["width"]:.1f} cm)')

      # Mark peak
      center_idx = int(np.mean(tire['indices']))
      plt.plot(t[center_idx], tire['width'], 'o', color=color, markersize=8)
      plt.text(t[center_idx], tire['width'] + 1, f'{tire["width"]:.1f} cm\n{tire["duration"]:.2f}s', ha='center', va='bottom', color=color, fontweight='bold')
    ...
    values = [details['front_width'], details['rear_width'], details['avg_duration']*10]  # Scale for visibility
